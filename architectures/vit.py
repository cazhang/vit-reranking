...
class Network(nn.Module):
	def __init__(self, opt):
		...
		self.pars = opt
		#self.vit_name = 'deit_base_patch16_224'
		self.vit_name = 'deit_small_patch16_224'
		self.last_in = 768
		self.model = timm.create_model(self.vit_name, pretrained=False)
		del self.model.head
		if self.vit_name == 'deit_base_patch16_224':
			checkpoint_url = '/home/czhang/Pretrained_models/Transformers/deit_base_patch16_224-b5f2ef4d.pth'
			self.last_in = 768
		elif self.vit_name == 'deit_small_patch16_224':
			checkpoint_url = '/home/czhang/Pretrained_models/Transformers/deit_small_patch16_224-cd65a155.pth'
			self.last_in = 384
		else:
			raise ValueError('Deit ckpt not found.')
	...
	def forward(self, x, **kwargs):
		...
		cls_token = self.model.cls_token.expand(x.shape[0], -1, -1)  # (n_samples, 1, embed_dim)
		x = torch.cat((cls_token, x), dim=1)  # (n_samples, 1 + n_patches, embed_dim)
		x = x + self.model.pos_embed  # (n_samples, 1 + n_patches, embed_dim)
		x = self.model.pos_drop(x)
		for i, blk in enumerate(self.model.blocks):
			x = blk(x)
		x = self.model.norm(x)
		enc_out = x[:,0]
		no_avg_feat = x[:, 1::] #.permute(0,2,1) # n, patch, dim
		#no_avg_feat = no_avg_feat.view(x.size(0), -1, int(no_avg_feat.size(-1)**0.5), int(no_avg_feat.size(-1)**0.5))
		x = self.model.head(enc_out)
