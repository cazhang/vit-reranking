...
class Criterion(torch.nn.Module):
	def __init__(self, opt, batchminer):
		...
		self.use_cls_token = opt.use_cls_token
		self.temperature = opt.temperature
		self.use_inverse = opt.use_inverse
	...
	def cross_attention(self, x, y, x_mean, y_mean):
		...
		#att = torch.exp(-att / self.temperature) if self.use_inverse else att # old
		att = 1-att if self.use_inverse else att
	...
	def pair_wise_wdist(self, x, y, x_mean=None, y_mean=None, is_positive=True):
		B, C, H, W = x.size()
		x = x.view(B, C, -1, 1)
		y = y.view(B, C, 1, -1)
		if (x_mean is None) and (y_mean is None):
			x_mean = x.mean([2, 3])
			y_mean = y.mean([2, 3])
		...
		sim = torch.einsum('bcs, bcm->bsm', x, y).contiguous() # original
		...
		if is_positive:
			with torch.no_grad():
				...
				T = self.Sinkhorn(K, v, u) # maybe wrong order, should be v and u
		else:
			T = torch.ones_like(wdist)
	...
	def forward(self, batch, labels, **kwargs):
		...
		if self.use_cls_token:
			assert 'class_token' in kwargs.keys(), f"class_token not found"
			pooled_feature = kwargs['class_token']
		else:
		...
		if len(sampled_triplets):
			...
			for triplet in sampled_triplets:
				...
				if self.use_cls_token:
					class_triplet = {'Anchor': pooled_feature[triplet[0]], 'Positive':pooled_feature[triplet[1]], 'Negative':pooled_feature[triplet[2]]}
					pos_dist = self.pair_wise_wdist(train_triplet['Anchor'].unsqueeze(0),
													train_triplet['Positive'].unsqueeze(0),
													class_triplet['Anchor'].unsqueeze(0),
													class_triplet['Positive'].unsqueeze(0),
													)
					neg_dist = self.pair_wise_wdist(train_triplet['Anchor'].unsqueeze(0),
													train_triplet['Negative'].unsqueeze(0),
													class_triplet['Anchor'].unsqueeze(0),
													class_triplet['Negative'].unsqueeze(0),
													)
				else:
