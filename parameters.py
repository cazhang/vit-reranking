...
def basic_training_parameters(parser):
	...
	parser.add_argument('--eval_epochs', default=1, type=int, help='Number of evaluation frequence.')
	...
	parser.add_argument('--cachebatchsize', default=20, type=int, help='resume path')
	parser.add_argument('--lrstep', default=20, type=int, help='resume path')
	parser.add_argument('--lrgamma', default=0.5, type=float, help='resume path')
	parser.add_argument('--weightdecay', default=0.001, type=float, help='resume path')
	parser.add_argument('--momentum', default=0.9, type=float, help='resume path')
	parser.add_argument('--patience', default=10, type=int, help='resume path')
	parser.add_argument('--margin', default=0.1, type=float, help='resume path')
	parser.add_argument('--nNeg', default=5, type=int, help='resume path')
	parser.add_argument('--imageresizew', default=640, type=int, help='resume path')
	parser.add_argument('--imageresizeh', default=480, type=int, help='resume path')
	parser.add_argument('--start_epoch', default=0, type=int, help='resume path')
	parser.add_argument('--evalevery', default=10, type=int, help='resume path')
	parser.add_argument('--max_patience', default=100, type=int, help='max patience')
	...
	parser.add_argument('--num_classes', default=1000, type=int, help='output dimension')
	...
	parser.add_argument('--not_pretrained',   action='store_true',							  help='Flag. If set, ImageNet pretraining is used to initialize the network.')
	...
	##### ViT Evaluation Parameters
	parser.add_argument('--blk_ind', default=0, type=int, help='Index of transformer block used for q and k.')
	parser.add_argument('--cross_attn_depth', default=1, type=int, help='Layers of cross vit.')
	parser.add_argument('--skip_last_vit_norm', default=False, action='store_true')
	parser.add_argument('--use_pretrained', default=False, action='store_true')
	parser.add_argument('--cat_global', default=False, action='store_true')

	parser.add_argument('--warmup_epoch', default=30, type=int, help='Epochs to warmup gnn cross layers.')
	parser.add_argument('--grid_size', default=7, type=int, help='Epochs to warmup gnn cross layers.')
	parser.add_argument('--use_cls_token', default=False, action='store_true',
						help='If set, class token is used as global feature for cross correlation')
	parser.add_argument('--use_uniform', default=False, action='store_true',
						help='If set, correlation will be negated first and then exponential using a temperature')
	parser.add_argument('--use_inverse', default=False, action='store_true', help='If set, correlation will be negated first and then exponential using a temperature')
	parser.add_argument('--use_minus', default=False, action='store_true',
						help='If set, correlation will be negated first and then exponential using a temperature')
	parser.add_argument('--use_soft', default=False, action='store_true',
						help='If set, correlation will be negated first and then exponential using a temperature')
	parser.add_argument('--use_rollout', default=False, action='store_true',
						help='If set, correlation will be negated first and then exponential using a temperature')
	parser.add_argument('--use_ot', default=False, action='store_true',
						help='If set, OT is used, otherwise, dual softmax')
	parser.add_argument('--use_relu', default=False, action='store_true',
						help='If set, Cross-correlation is ReLu')
	parser.add_argument('--training', default=False, action='store_true',
						help='If set, OT is used, otherwise, dual softmax')
	parser.add_argument('--shuffle', default=False, action='store_true',
						help='If set,dataloader is shuffled')
	parser.add_argument('--temperature', default=0.1, type=float, help='Temperature argument used in exp. Large value means smooth distribution')
	parser.add_argument('--ot_part', default=1.0, type=float, help='Amount of mass to be transported: [0, 1]')
	parser.add_argument('--to_submit', default=False, action='store_true',
						help='If set, making figures in submit mode: fixed ranking')
	parser.add_argument('--plot_topk', default=1, type=int, help='Epochs to warmup gnn cross layers.')
	parser.add_argument('--task_loss', default='triplet', type=str, help='Loss for task.')
	parser.add_argument('--distill', default=None, type=str, help='distill type')
	parser.add_argument('--distill_hardneg', default=False, action='store_true', help='use teacher for hard neg mining')
	parser.add_argument('--rand_distill', default=False, action='store_true')
	parser.add_argument('--kd_weight', default=1.0, type=float, help='KD loss weight')
	parser.add_argument('--rkd_distance_weight', default=1.0, type=float, help='KD loss weight')
	parser.add_argument('--rkd_angle_weight', default=1.0, type=float, help='KD loss weight')
	parser.add_argument('--tl_weight', default=1.0, type=float, help='Triplet loss weight')
	parser.add_argument('--supcon_temp', default=0.1, type=float, help='contrastive loss temperature')

	parser.add_argument('--rand_triplets', default=False, action='store_true')
	parser.add_argument('--is_parallel', default=False, action='store_true')
	parser.add_argument('--save_freq', default=100, type=int, help='Frequence to save retrieval results.')
	parser.add_argument('--vis_freq', default=1, type=int, help='Frequence to save heatmap for DIML.')

	parser.add_argument('--debug', default=False, action='store_true')

	##### NetVLAD Parameters
	parser.add_argument('--mini_data', default=False, action='store_true')
	parser.add_argument('--vlad_ckpt', default='/home/czhang/Pretrained_models/Netvlad/pittsburgh_WPCA4096.pth.tar', type=str,   help='Netvlad pretrained ckpt.')
	parser.add_argument('--pooling', default='patchnetvlad', type=str, help='Layers of cross vit.')
	parser.add_argument('--num_clusters', default=64, type=int, help='Layers of cross vit.')
	parser.add_argument('--patch_sizes', default='5', type=str, help='Layers of cross vit.')
	parser.add_argument('--strides', default='0', type=str, help='Layers of cross vit.')
	parser.add_argument('--append_pca_layer', default=False, action='store_true')
	parser.add_argument('--vladv2', default=False, action='store_true')
	parser.add_argument('--num_pcs', default=4096, type=int, help='number of pcs of PCA')
	...
	##### XBM Parameters
	parser.add_argument('--enable_xbm', default=False, action='store_true')
	parser.add_argument('--xbm_start_iteration', default=1000, type=int, help='Frequence to save retrieval results.')
	parser.add_argument('--xbm_size', default=10000, type=int, help='Frequence to save retrieval results.')
	parser.add_argument('--xbm_weight', default=1.0, type=float, help='contrastive loss temperature')
