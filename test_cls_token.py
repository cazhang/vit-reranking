...
# prepare path
CUB_LOGS = {
	'Margin_b06_cvt': ['CUB_Margin_b06_Distance_s0_cvt'],
}
...
from proofs.vit_vs_cnn import evaluate_cls_cluster, plot_cluster
...
for method, info in LOGS.items():
	...
	if MODE_DEBUG:  # finetuned version
		data, labels = evaluate_cls_cluster(model, datasets['testing'], dataloaders['testing'], opt.training, trunc_nums=trunc_nums, use_uniform=opt.use_uniform, grid_size=opt.grid_size, to_submit=opt.to_submit)
		plot_cluster(data, labels, opt)
