...
from evaluation.eval_cvt_diml import evaluate, evaluate_patch_similarity
...
dataloaders['testing']	= torch.utils.data.DataLoader(datasets['testing'],	num_workers=opt.kernels, batch_size=opt.bs, shuffle=opt.shuffle)
...
# prepare path
CUB_LOGS = {
	'Margin_b06_cvt': ['CUB_Margin_b06_Distance_s0']
}
...
for method, info in LOGS.items():
	if opt.pretrained:
		ckpt_path = f'Training_Results/{opt.dataset}/{info[0]}/best.pth'
		best_metrics = load_checkpoint(model, None, ckpt_path)
		print(best_metrics)
	
	result = evaluate(model, datasets['testing'], dataloaders['testing'], opt.training, trunc_nums,
					  use_uniform=opt.use_uniform,
					  grid_size=opt.grid_size,
					  use_inverse=opt.use_inverse,
					  temperature=opt.temperature,
					  use_cls_token=opt.use_cls_token,
					  attn_blk_ind=opt.blk_ind,
					  use_ot=opt.use_ot,
					  ot_part=opt.ot_part,
					  to_submit=opt.to_submit,
					  use_minus=opt.use_minus,
					  use_rollout=opt.use_rollout,
					  plot_topk=opt.plot_topk
					  )
