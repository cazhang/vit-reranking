...
from evaluation.eval_swin_diml import evaluate, evaluate_patch_similarity
...
# prepare path
CUB_LOGS = {
	'Margin_b06_swin_tiny': ['CUB_Margin_b06_Distance_s0_swin_tiny']
}

CARS_LOGS = {
	'Margin_b06_vit_v2': ['CARS_Margin_b06_Distance_s0_vit'],
...
for method, info in LOGS.items():
	...
	result = evaluate(model, datasets['testing'], dataloaders['testing'], training=opt.training, trunc_nums=trunc_nums,
					  use_uniform=opt.use_uniform, grid_size=opt.grid_size,
					  blk_ind=opt.blk_ind, use_cls_token=opt.use_cls_token, to_submit=opt.to_submit)

	print(result)
	result['method'] = [f'{method} + block {opt.blk_ind} + ours ({trunc})' for trunc in trunc_nums]
