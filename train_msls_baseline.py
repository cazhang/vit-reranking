...
'''
This code trains the CVT on MSLS.
...
if __name__ == "__main__":
	...
	################### INPUT ARGUMENTS ###################
	parser = argparse.ArgumentParser(description='MSLS-CVT-train')
	...
	if opt.resume_path:
		logging_logger.info(f'Loading checkpoint from {opt.resume_path} ...')
	...
	if opt.distill is not None:
		teacher_model = teacher_model.to(device)
		criterion_distill = nn.SmoothL1Loss(reduction='mean').to(device)
	else:
		teacher_model = None
	...
	if opt.resume_path:
		...
		best_recalls = checkpoint['recalls']
	...
	for epoch in trange(opt.start_epoch, opt.n_epochs, desc='Epoch number'.rjust(15), position=0):
		...
		train_loss = train_epoch(train_dataset, model, optimizer, criterion, device, epoch, opt, logging_logger, LOG, teacher_model, criterion_distill)
